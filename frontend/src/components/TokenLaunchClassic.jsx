  ...
  return (
    <div className="relative inline-block">
      ...
      {show && createPortal(
        <div 
          ref={tooltipRef}
          className="fixed z-[9999] w-72 p-3 bg-gray-900 border border-gray-700 rounded-lg shadow-sm text-xs"
          style={{
          ...
          <div className="space-y-1.5">
            {typeof content === 'string' ? (
              <p className="text-gray-200 leading-relaxed flex items-start gap-2">
                <InformationCircleIcon className="w-4 h-4 text-purple-400 flex-shrink-0 mt-0.5" />
                <span>{content}</span>
              </p>
            ) : (
              <div className="space-y-1.5">
...
const CollapsibleInfo = ({ title, children, defaultOpen = false }) => {
  ...
  return (
    <div className="border-t border-gray-800 pt-2 mt-2">
      ...
      {isOpen && (
        <div className="mt-2 p-2 bg-gray-900 border border-gray-800 rounded text-xs text-gray-400">
          {children}
        </div>
      )}
    </div>
...
export default function TokenLaunchClassic({ onLaunch }) {
  ...
  const [walletSourceExpanded, setWalletSourceExpanded] = useState(false);
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      try {
        ...
        console.warn('[AutoSave] ?????? Failed to auto-save wallet selections:', error);
      }
    ...
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-4">
      <div className="flex justify-between items-center mb-2">
        <div className="flex items-center gap-3">
          <RocketLaunchIconSolid className="w-6 h-6 text-purple-400" />
          <h2 className="text-2xl font-bold text-white">Launch Token</h2>
          <span className="text-xs text-gray-500 bg-gray-800 px-2 py-0.5 rounded">Advanced</span>
        </div>
        {savingStatus && (
          <div className={`text-xs px-3 py-1 rounded-lg ${
            savingStatus.startsWith('[ok]') ? 'bg-green-500/10 text-green-400 border border-green-500/30' : 
            savingStatus.startsWith('[x]') ? 'bg-red-500/10 text-red-400 border border-red-500/30' : 
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            ...
            className={`p-4 rounded-xl border-2 transition-all text-left ${
              launchMode === 'bundle' ? 'border-purple-500 bg-purple-500/10' : 'border-gray-700 hover:border-gray-600 bg-gray-800/50'
            }`}
          >
            <div className="flex items-center gap-2 mb-1">
              <CubeIcon className={`w-5 h-5 ${launchMode === 'bundle' ? 'text-purple-400' : 'text-gray-400'}`} />
              <span className={`font-semibold ${launchMode === 'bundle' ? 'text-white' : 'text-gray-300'}`}>Bundle</span>
            </div>
          ...
          <button
            ...
            className={`p-4 rounded-xl border-2 transition-all text-left ${
              launchMode === 'rapid' ? 'border-purple-500 bg-purple-500/10' : 'border-gray-700 hover:border-gray-600 bg-gray-800/50'
            }`}
          >
            <div className="flex items-center gap-2 mb-1">
              <RocketLaunchIcon className={`w-5 h-5 ${launchMode === 'rapid' ? 'text-purple-400' : 'text-gray-400'}`} />
              <span className={`font-semibold ${launchMode === 'rapid' ? 'text-white' : 'text-gray-300'}`}>Quick</span>
            </div>
          ...
          <div className="flex items-center justify-between px-5 py-3 border-b border-gray-800">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-3 py-1.5 bg-gray-800 hover:bg-gray-700 text-gray-300 rounded-lg transition-colors text-xs font-medium"
              >
                Load Profile
              </button>
              <button
          ...
          <div className="px-5 py-3 border-b border-gray-800">
            <div className="flex items-center justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-xs font-medium text-gray-400">Contract Address</span>
                  {nextAddress?.address?.toLowerCase().endsWith('pump') && (
                    <span className="text-[10px] text-purple-400 bg-purple-500/10 px-1.5 py-0.5 rounded">vanity</span>
                  )}
                </div>
                {nextAddress?.address ? (
                  <code className="text-sm font-mono text-white break-all block">{nextAddress.address}</code>
                ) : (
                  <span className="text-xs text-gray-500">
                    {addressMode === 'vanity' && vanityAddressPool.available === 0
                      ? 'No vanity addresses — start generator or switch to Random'
                      : 'Loading...'}
                  </span>
                )}
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                <div className="flex items-center bg-gray-800 rounded-lg p-0.5">
                  <button type="button" onClick={() => setAddressMode('vanity')}
                    className={`px-2.5 py-1.5 rounded-md text-xs font-medium transition-all ${addressMode === 'vanity' ? 'bg-purple-600 text-white' : 'text-gray-400 hover:text-white'}`}>
                    Vanity
                  </button>
                  <button type="button" onClick={() => setAddressMode('random')}
                    className={`px-2.5 py-1.5 rounded-md text-xs font-medium transition-all ${addressMode === 'random' ? 'bg-purple-600 text-white' : 'text-gray-400 hover:text-white'}`}>
                    Random
                  </button>
                </div>
                {nextAddress?.address && (
                  <>
                    <button type="button" onClick={() => { navigator.clipboard.writeText(nextAddress.address); showToast('Copied!'); }}
                      className="p-2 bg-gray-800 hover:bg-gray-700 rounded-lg transition-all" title="Copy">
                      <svg className="w-4 h-4 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
                      </svg>
                    </button>
                    <a href={`https://pump.fun/${nextAddress.address}`} target="_blank" rel="noopener noreferrer"
                      className="p-2 bg-purple-500/10 hover:bg-purple-500/20 rounded-lg transition-all border border-purple-500/30"
                ...
                <div className="relative group">
                  {settings.FILE || imagePreview ? (
                    <img 
                      ...
                      className="w-32 h-32 rounded-xl object-cover border-2 border-gray-800 group-hover:border-blue-500/50 transition-colors"
                    />
                  ) : (
              ...
              <div className="flex-1 space-y-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div className="col-span-2">
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 text-sm bg-gray-800/50 border border-gray-800 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full px-3 py-2 text-sm bg-gray-800/50 border border-gray-800 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 resize-none"
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button
              ...
              className="px-3 py-1.5 bg-purple-600 hover:bg-purple-500 text-white rounded-lg transition-colors flex items-center gap-2 text-xs font-semibold shadow-lg shadow-purple-600/20"
            ...
            <button
              ...
              className="px-3 py-1.5 bg-green-600 hover:bg-green-700 text-white rounded-lg transition-colors flex items-center gap-2 text-xs font-semibold shadow-sm"
        ...
        {/* ????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????? */}
        <div className="mb-4 p-4 bg-gradient-to-r from-blue-900/30 via-purple-900/30 to-green-900/30 rounded-xl border border-purple-500/30 shadow-lg">
          {/* Header Row */}
            ...
            <div className="space-y-1">
              ...
              {walletInfo?.fundingWallet?.address && (
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className="text-[11px] font-mono text-purple-400">{walletInfo.fundingWallet.address}</span>
                  <button
                    type="button"
                    onClick={() => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
          ...
          <div className="hidden p-3 bg-gray-900 rounded-xl border border-gray-800">
            <label className="block text-base font-semibold text-purple-400 mb-2 flex items-center gap-1">
            ...
            <div className="space-y-2">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 p-2 bg-gray-800/50 rounded border border-gray-800">
                  <input
                    type="checkbox"
          ...
          {/* DEV Wallet */}
          <div className="lg:col-span-2 p-5 bg-gray-900 rounded-xl border border-gray-800">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <UserIcon className="w-5 h-5 text-purple-400" />
                <span className="text-base font-bold text-white">DEV Wallet</span>
                <span className="text-[10px] px-2 py-0.5 bg-purple-500/10 text-purple-400 rounded">Creates Token</span>
              </div>
            </div>
            ...
            <div className="mb-4 p-3 bg-gray-800/30 rounded-lg border border-gray-800">
              <div className="flex items-center justify-between mb-2">
                ...
                {(settings.USE_FUNDING_AS_BUYER === 'true' || settings.BUYER_WALLET === settings.PRIVATE_KEY) ? (
                  <span className="text-[10px] font-mono text-purple-400">Master Wallet</span>
              ...
              {(settings.USE_FUNDING_AS_BUYER === 'true' || settings.BUYER_WALLET === settings.PRIVATE_KEY) && (
                <p className="mt-2 text-[10px] text-purple-400">??? Using your Master Wallet as the token creator</p>
              )}
            </div>
            ...
            {(() => {
              ...
              <div>
                        <div className="flex items-center gap-2">
                          ...
                          {isUsingMaster && (
                            <span className="text-[9px] px-1.5 py-0.5 bg-purple-600/30 text-purple-400 rounded font-medium">MASTER</span>
                          )}
                          {isUsingExisting && (
                ...
                />
                  ...
                  {/* Auto-Sell Toggle */}
                  <div className={`p-3 rounded-lg border transition-all ${isDevAutoSellEnabled ? 'bg-red-900/30 border-red-600/50' : 'bg-gray-900 border-gray-700 hover:border-gray-600'}`}>
                    <div className="flex items-center justify-between mb-2">
          ...
          <div className="p-5 bg-gray-900 rounded-xl border border-gray-800">
            <div className="flex items-center justify-between mb-4 gap-2">
              <div className="flex items-center gap-2 min-w-0">
                <CubeIcon className="w-5 h-5 text-purple-400 flex-shrink-0" />
                <span className="text-base font-bold text-white">Bundle Wallets</span>
                <span className="text-[10px] px-2 py-0.5 bg-green-500/10 text-green-400 rounded whitespace-nowrap flex-shrink-0">Same block as DEV</span>
              </div>
                            ...
                            {selectedBundleWallets.map((addr, i) => {
                              ...
                              return (
                                <div key={addr} className={`p-4 rounded-lg border transition-all ${isAutoSellEnabled ? 'bg-green-900/20 border-green-600/50' : 'bg-gray-800/50 border-gray-700'}`}>
                                  ...
                                  {/* Auto-Sell Toggle */}
                                  <div className={`p-3 rounded-lg border transition-all ${isAutoSellEnabled ? 'bg-red-900/30 border-red-600/50' : 'bg-gray-900 border-gray-700 hover:border-gray-600'}`}>
                                    <div className="flex items-center justify-between mb-2">
                                      <label className="text-xs font-semibold text-gray-300 flex items-center gap-1.5">
                            ...
                            {additionalBundleCount > 0 && Array.from({ length: additionalBundleCount }, (_, i) => {
                              ...
                              return (
                                <div key={walletId} className={`p-4 rounded-lg border transition-all ${isAutoSellEnabled ? 'bg-blue-900/20 border-blue-600/50' : 'bg-gray-800/50 border-gray-700'}`}>
                                  <div className="flex items-center justify-between mb-3">
                                    <div className="flex items-center gap-2">
                                      <div className={`p-1.5 rounded ${isAutoSellEnabled ? 'bg-purple-500/10' : 'bg-gray-700'}`}>
                                        <WalletIcon className={`w-4 h-4 ${isAutoSellEnabled ? 'text-purple-400' : 'text-gray-400'}`} />
                                      </div>
                                      <div>
                                        <div className="flex items-center gap-2">
                                          <span className="text-sm font-bold text-purple-400">Bundle #{walletIdx}</span>
                                          <span className="text-[9px] px-1.5 py-0.5 bg-purple-600/30 text-purple-400 rounded font-medium">AUTO-CREATED</span>
                                        </div>
              ...
              <div className="flex items-center gap-2 pt-2 border-t border-gray-800">
                <div className={`px-2 py-1 rounded text-xs font-medium ${
                  launchMode === 'rapid' 
                    ...
                    : launchMode === 'bundle'
                    ? 'bg-blue-900/30 text-purple-400 border border-purple-500/30'
          ...
          <div className="p-5 bg-gray-900 rounded-xl border border-gray-800">
            <div className="flex items-center justify-between mb-4 gap-2">
              <div className="flex items-center gap-2 min-w-0">
                <UserGroupIcon className="w-5 h-5 text-purple-400 flex-shrink-0" />
                <span className="text-base font-bold text-white">Holder Wallets</span>
              </div>
              <span className="text-xs text-gray-400 whitespace-nowrap flex-shrink-0">
                {useWarmedHolderWallets 
                      ...
                      {selectedHolderWallets.map((addr, i) => {
                        ...
                        return (
                          <div key={addr} className={`p-4 rounded-lg border transition-all ${isAutoBuyEnabled || isAutoSellEnabled ? 'bg-yellow-900/20 border-yellow-600/50' : 'bg-gray-800/50 border-gray-700'}`}>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                              {/* Auto-Buy Toggle */}
                              <div className={`p-3 rounded-lg border transition-all ${isAutoBuyEnabled ? 'bg-green-900/30 border-green-600/50' : 'bg-gray-900 border-gray-700 hover:border-gray-600'}`}>
                                <div className="flex items-center justify-between mb-2">
                              ...
                              <div>
                                      ...
                                      <div className="flex gap-1">
                                        ...
                                        <button type="button" onClick={() => { const n = { ...holderAutoBuyConfigs }; n[addr] = { ...config, delay: 1 }; setHolderAutoBuyConfigs(n); }} className="px-2 py-1 text-[10px] bg-purple-600 hover:bg-purple-500 text-white rounded">1s</button>
                                      </div>
                                    </div>
                                    <div>
                      ...
                      {additionalHolderCount > 0 && Array.from({ length: additionalHolderCount }, (_, i) => {
                        ...
                        return (
                          <div key={walletId} className={`p-4 rounded-lg border transition-all ${isAutoBuyEnabled || isAutoSellEnabled ? 'bg-blue-900/20 border-blue-600/50' : 'bg-gray-800/50 border-gray-700'}`}>
                            ...
                            <div className="flex items-center justify-between mb-3">
                              <div className="flex items-center gap-2">
                                <div className={`p-1.5 rounded ${isAutoBuyEnabled || isAutoSellEnabled ? 'bg-purple-500/10' : 'bg-gray-700'}`}>
                                  <WalletIcon className={`w-4 h-4 ${isAutoBuyEnabled || isAutoSellEnabled ? 'text-purple-400' : 'text-gray-400'}`} />
                                </div>
                                  <div>
                                  <div className="flex items-center gap-2">
                                    <span className="text-sm font-bold text-purple-400">Holder #{walletIdx}</span>
                                    <span className="text-[9px] px-1.5 py-0.5 bg-purple-600/30 text-purple-400 rounded font-medium">AUTO-CREATED</span>
                                  </div>
                                    ...
                                    <div className="flex gap-1">
                                      <input
                                        ...
                                        onChange={(e) => {
                                            ...
                                            <button type="button" onClick={() => { const n = { ...holderAutoBuyConfigs }; n[walletId] = { ...config, delay: 1 }; setHolderAutoBuyConfigs(n); }} className="px-2 py-1 text-[10px] bg-purple-600 hover:bg-purple-500 text-white rounded">1s</button>
                                    </div>
                                  </div>
                                      <div>
                                          <label className="block text-[10px] text-gray-500 mb-1 flex items-center gap-1">
      ...
      {showWalletModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-gray-900 border border-gray-700 rounded-xl w-full max-w-4xl max-h-[85vh] flex flex-col shadow-sm">
                {/* Modal Header */}
                            ...
                            <div className="space-y-1 text-xs mb-3">
                              ...
                              {wallet.tags && wallet.tags.length > 0 && (
                                <div className="flex flex-wrap gap-1 mt-1">
                                  {wallet.tags.map(tag => (
                                    <span key={tag} className="px-1.5 py-0.5 bg-blue-900/50 text-purple-300 rounded text-sm">
                                      {tag}
                                    </span>
                                  ))}
                                </div>
                ...
                <div className="flex items-center justify-between p-4 border-t border-gray-800 bg-gray-900">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={() => setShowWalletModal(false)}
                      className="px-6 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-lg text-sm font-medium"
                    >
                      Done
                    </button>
                  </div>
      ...
      <div className="mb-3">
        <button
          ...
          disabled={loading}
          className="w-full py-3 bg-purple-600 hover:bg-purple-500 text-white font-bold rounded-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed glow-blue flex items-center justify-center gap-2"
      ...
      <div className="mt-6 mb-6">
        <button
          ...
          className={`w-full py-4 text-white font-black text-xl rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-sm flex items-center justify-center gap-3 relative overflow-hidden group ${
            launchMode === 'rapid'
      ...
      {walletInfo && (
        <div className="mt-6 space-y-4" data-section="wallet-config">
            {/* Funding Wallet */}
            <div className="p-4 bg-gray-900 rounded-xl border border-gray-800 transition-all duration-300">
            ...
            {(walletInfo.bundleWallets.count > 0 || (useWarmedBundleWallets && selectedBundleWallets.length > 0)) && (
              <div className="p-4 bg-gray-900 rounded-xl border border-gray-800">
                ...
                {walletInfo.bundleWallets.totalSol > 0 && (
                  <p className="text-[10px] text-purple-400 mt-1">??? Will be funded from Master Wallet during initialization</p>
                )}
            ...
            {(walletInfo.holderWallets.count > 0 || (useWarmedHolderWallets && selectedHolderWallets.length > 0)) && (
              <div className={`p-4 bg-gray-900 rounded-lg border-l-4 ${useWarmedHolderWallets && selectedHolderWallets.length > 0 && walletInfo.holderWallets.totalSol === 0 ? 'border-green-500' : 'border-yellow-500'}`}>
                <div className="flex justify-between items-start mb-2">
                  <div>
              ...
              {((useWarmedHolderWallets && selectedHolderAutoBuyWallets.length > 0) || (!useWarmedHolderWallets && selectedHolderAutoBuyIndices.length > 0)) && (
                <div className="pt-4 border-t border-gray-700">
                                ...
                                <div className="flex gap-1">
                                  ...
                                  <div className="flex flex-col gap-0.5">
                                    ...
                                    <button
                                      ...
                                      className="px-2 py-0.5 text-[10px] bg-purple-600 hover:bg-purple-500 text-white rounded"
                  ...
                  <div className="mt-3 p-3 bg-blue-900/20 border border-blue-700/50 rounded">
                    <p className="text-xs text-purple-300 font-semibold mb-1">???? Tips:</p>
            ...
            <div className="p-4 overflow-y-auto flex-1">
              ...
              {configModalTab === 'token' && (
                <>
                  ...
                  <div className="mb-3 p-3 bg-gray-800/30 rounded-lg border border-gray-800">
                    <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="mb-3 p-3 bg-gray-800/30 rounded-lg border border-gray-800">
                    ...
                    <label className="block w-full px-3 py-2 bg-purple-600 hover:bg-purple-500 text-white text-sm rounded-lg transition-colors flex items-center justify-center gap-2 cursor-pointer">
                      <ArrowPathIcon className="w-4 h-4" />
                      Choose JSON File
                      <input
                        type="file"
                        ...
                        <div className="flex gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              {config.symbol && (
                                <span className="text-xs px-2 py-0.5 bg-blue-900/50 text-purple-300 rounded flex-shrink-0">
                                  {config.symbol}
                                </span>
                              )}
                            </div>
                            ...
                            <div className="flex flex-wrap gap-2 mb-2">
                              ...
                              {config.data?.TELEGRAM && (
                                <a 
                                  ...
                                  rel="noopener noreferrer"
                                  onClick={(e) => e.stopPropagation()}
                                  className="text-xs px-2 py-0.5 bg-blue-900/50 text-purple-300 rounded hover:bg-blue-800/50 transition-colors"
                                >
                                   Telegram
                                </a>
                              )}
      ...
      {showTotalSolModal && walletInfo && walletInfo.breakdown && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-gray-900 rounded-xl border-2 border-yellow-500/50 shadow-sm max-w-md w-full max-h-[90vh] overflow-y-auto my-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
          ...
          <div
            ...
            className={`px-4 py-3 rounded-lg shadow-lg border backdrop-blur-sm animate-slide-in flex items-center gap-2 min-w-[200px] ${
              toast.type === 'success' 
                ...
                : 'bg-blue-900/90 border-blue-500/50 text-purple-300'
            }`}
          >
            {toast.type === 'success' && (
